...
class SimpleO3 final : public IFrontEnd, public Implementation {
  ...
  public:
    void init() override {
      ...
      int ipc   = param<int>("ipc").desc("IPC of the SimpleO3 core.").default_val(16);
      int depth = param<int>("inst_window_depth").desc("Instruction window size of the SimpleO3 core.").default_val(1024);
      ...
      int llc_associativity     = param<int>("llc_associativity").desc("LLC set associativity.").default_val(1);
      int llc_capacity_per_core = parse_capacity_str(param<std::string>("llc_capacity_per_core").desc("LLC capacity per core.").default_val("1KB"));
      int llc_num_mshr_per_core = param<int>("llc_num_mshr_per_core").desc("Number of LLC MSHR entries per core.").default_val(1024);
