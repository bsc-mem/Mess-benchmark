...
int RequireRowOpen(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
  ...
  if (node->m_state == T::m_states["Closed"]) return T::m_commands["ACT"];
  ...
  if (node->m_state == T::m_states["Refreshing"]) return T::m_commands["ACT"];
...
int RequireBankClosed(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
  ...
  if (node->m_state == T::m_states["Closed"]) return cmd;
  if (node->m_state == T::m_states["Opened"]) return T::m_commands["PRE"];
  if (node->m_state == T::m_states["Refreshing"]) return cmd;
...
int RequireAllBanksClosed(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
  if  (T::m_levels["bank"] - T::m_levels["rank"] == 1) {
  ...
  } else if (T::m_levels["bank"] - T::m_levels["rank"] == 2) {
...
namespace Channel {
  template <class T>
  int RequireAllBanksClosed(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
    if (T::m_levels["bank"] - T::m_levels["channel"] == 2) {
      for (auto bg : node->m_child_nodes) {
        for (auto bank: bg->m_child_nodes) {
          if (bank->m_state == T::m_states["Closed"]) {
            continue;
    ...
    } else if (T::m_levels["bank"] - T::m_levels["channel"] == 3) {
