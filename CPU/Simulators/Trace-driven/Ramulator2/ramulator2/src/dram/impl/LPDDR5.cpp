  ...
  private:
    ...
    void set_preqs() {
        ...
        for (auto bg : node->m_child_nodes) {
          for (auto bank : bg->m_child_nodes) {
            ...
            if (flat_bankid == addr_vec[LPDDR5::m_levels["bank"]] || flat_bankid == addr_vec[LPDDR5::m_levels["bank"]] + 8) {
              if (node->m_state == m_states["Pre-Opened"]) return m_commands["PRE"];
              if (node->m_state == m_states["Opened"]) return m_commands["PRE"];
            }
          }
        }
      ...
      m_preqs[m_levels["bank"]][m_commands["RD16"]] = [] (Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
        ...
        if (node->m_state == m_states["Closed"]) return m_commands["ACT-1"];
        if (node->m_state == m_states["Pre-Opened"]) return m_commands["ACT-2"];
        if (node->m_state == m_states["Opened"]) {
