...
template <class T>
void populate_timingcons(T *spec,
                         std::vector<TimingConsInitializer> initializer) {
  spec->m_timing_cons.resize(
      T::m_levels.size(),
      std::vector<std::vector<TimingConsEntry>>(T::m_commands.size()));
  for (const auto &ts : initializer) {
    ...
    for (auto p_cmd_str : ts.preceding) {
      ...
      for (auto f_cmd_str : ts.following) {
        ...
        spec->m_timing_cons[level][p_cmd].push_back(
            {f_cmd, ts.latency, ts.window, ts.is_sibling});
      }
    }
  }
};

template <int N> class ImplDef;
...
public:
  SpecDef() = default;
  template <int N>
  constexpr SpecDef(const ImplDef<N> &spec)
      : std::vector<std::string_view>(spec.begin(), spec.end()) {
    for (int i = 0; i < spec.size(); i++) {
      m_str2int_map[spec.std::template array<std::string_view, N>::operator[](
          i)] = i;
    }
  }
  ...
  std::string_view operator()(int i) const { return operator[](i); };
  int operator()(std::string_view name) const { return operator[](name); };
...
template <typename... Ts> ImplDef(Ts &&...elems) -> ImplDef<sizeof...(Ts)>;

template <int N, int M, typename V> class ImplLUT;

template <typename V = int> class SpecLUT : public std::vector<V> {
private:
...
public:
  ...
  template <int N, int M> SpecLUT &operator=(const ImplLUT<N, M, V> &spec) {
  ...
  V &operator()(std::string_view name) { return operator[](name); }
  V &operator()(int id) { return operator[](id); }
...
template <int N, int M, typename V> class ImplLUT : public std::array<V, N> {
private:
...
template <int N, int M> class ImplLUT<N, M, int> : public std::array<int, N> {
private:
  const ImplDef<N> *m_key_def = nullptr;
...
public:
  ...
  ImplLUT(const ImplDef<N> &key_def, const ImplDef<M> &value_def,
...
ImplLUT(ImplDef<N>, ImplDef<M>,
        std::map<std::string_view, std::string_view>) -> ImplLUT<N, M, int>;

template <typename V, int N>
ImplLUT<N, 0, V> LUT(const ImplDef<N> &key_def,
                     const std::map<std::string_view, V> &lut) {
  return ImplLUT<N, 0, V>(key_def, lut);
};

template <int N, int M>
ImplLUT<N, M, int>
LUT(const ImplDef<N> &key_def, const ImplDef<M> &value_def,
    const std::map<std::string_view, std::string_view> &lut) {
...
public:
  ...
  enum class PowerState { IDLE = 0, ACTIVE = 1, REFRESHING = 2 };
