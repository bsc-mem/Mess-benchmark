...
def parse_results_file(filename, measurment_dir_name):
    ...
    with open(filename, 'r') as f:
        ...
        for line in lines:
            ...
            if tokens[0] == "ramulator.cpu_cycles":
                cpu_cycles = float(tokens[1])
            elif tokens[0] == "ramulator.read_latency_avg_0":
                read_latency = float(tokens[1])
            elif tokens[0] == "ramulator.read_requests":
                read_counts = float(tokens[1])
            elif tokens[0] == "ramulator.write_requests":
    ...
    bandwidth = frequency*64*(read_counts + write_counts) / cpu_cycles
...
def main(args): 
    ...
    results_files = [f"{base_directory}/{d}/DDR4.stats" for d in dirs]
